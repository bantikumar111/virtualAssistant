...
export const askToAssistant = async(req, res)=>{
  try {
    ...
    user.history.push(command) //save the history in db
    user.save()
    ...
    if(!jsonMatch){
      return res.status(400).json({response:"sorry, i didn't understant"})
    }
    ...
    return res.status(500).json({response:"ask assistant err"})
  }
}